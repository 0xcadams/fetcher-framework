...
import com.rentworthy.fetcher.response.source.DualSource;
...
public class DualConcurrentCachingFetcherWrapper<T> implements Fetcher<DualFetcherResponse<T>> {
     ...
     */
    public DualConcurrentCachingFetcherWrapper(final ConcurrentCachingFetcherWrapper<T> fetcherPrimary,
                                               final ConcurrentCachingFetcherWrapper<T> fetcherSecondary) {
        this(fetcherPrimary,
             fetcherSecondary,
             DualConcurrentCachingFetcherWrapper.DEFAULT_ERROR_CALLBACK,
             DualConcurrentCachingFetcherWrapper.DEFAULT_TIMEOUT_CALLBACK,
    ...
    public synchronized DualFetcherResponse<T> fetch() throws FetcherNotReadyException {
        ...
        if (this.countAttempts.getAndIncrement() >= this.maxAttempts) {
            this.timeoutCallback.onError(
                new FetcherNotReadyException("Max attempts reached for SubstitutingCachingFetcherWrapper"));
        }
